...
function checkSpambots($mail,$ip,$name){
	...
	if($sSFSAPI==''){
		$sTD_Results = $sTD_Results . '<tr><td class="results_white" colspan="2"><span class="error">'.$sSFSAPIError.'</span></td></tr>';
	}else{
	...
	if($sBSAPI==""){
		$sTD_Results = $sTD_Results . '<tr><td class="results_white" colspan="2"><span class="error">'.$sBSAPIError.'</span></td></tr>';
	}else{
		if($mail !=''){
	...
	if ($ip !=''){
		...
		if(!$sPHPAPI ==''){
			...
			$sTD_Results = $sTD_Results . '<tr><td align="left" width="200px" class="results"><a class="results" href="http://www.projecthoneypot.org/">ProjectHoneyPot</a>:</td><td class="results_white"><span class="error">'.$sPHPAPIError.'</span></td></tr>';
		} // End If
		...
		{
			$sdronespambot = true;
       			$sTD_Results = $sTD_Results . '<tr><td width="100px" class="results"><a class="results" href="http://dronebl.org/">DroneBL</a>:</td><td class="results_white"><span style="color: #ff0000">'.$ipfound.'</span></td></tr>';
		}else{
		...
		if ($lookup !=$sBLDETemp)
		{
			switch($sBLDETemp){
				...
				if($sBLDEspambot == true){
	       				$sTD_Results = $sTD_Results . '<tr><td width="100px" class="results"><a class="results" href="http://blocklist.de/">Blocklist.de</a>:</td><td class="results_white"><span style="color: #ff0000">'.$ipfound.' ('.$sBLDE.')</span></td></tr>';
				}else{
					$sTD_Results = $sTD_Results . '<tr><td width="100px" class="results"><a class="results" href="http://blocklist.de/">Blocklist.de</a>:</td><td class="results_white">'.$nomatches.'</td></tr>';
				}
			}
		}else{
			$sTD_Results = $sTD_Results . '<tr><td width="100px" class="results"><a class="results" href="http://blocklist.de/">Blocklist.de</a>:</td><td class="results_white">'.$nomatches.'</td></tr>';
		...
		// ****************************************************************************
		if($sbldespambot == true || $sTorspambot == true || $sefnetspambot == true || $sstraptvospambot == true || $stvospambot == true || $sphpspambot ==true || $sorbsspambot ==true || $spamhausspambot ==true || $scopspambot==true || $sdronespambot ==true || $shttpblachspambot || $sdroneachspambot || $sspamachspambot || $szeusachspambot){
			$spambot = true; // Required seperately now that dumping to a text file is optional
		}
	}else{
