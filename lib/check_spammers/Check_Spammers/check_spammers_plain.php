...
function checkSpambots($mail,$ip,$name){
	...
	$sCheckDNSBL = isset($_GET['dbl']);

	// If we do want to check the DNS blacklists, lets see if there are any singled out
	//
	// Note, this is only used here, check_spammers.php doesn't include this (not
	// really necessary as it's not going to be used by scripts).
	if(isset($_GET['sh'])){$sNoCheckSpamHaus = $_GET['sh'];} // SpamHaus
	if(isset($_GET['sc'])){$sNoCheckSpamCop = $_GET['sc'];} // SpamCOp
	if(isset($_GET['sb'])){$sNoCheckSorbs = $_GET['sb'];} // Sorbs
	if(isset($_GET['ph'])){$sNoCheckPHP = $_GET['ph'];} // Project Honey Pot
	if(isset($_GET['drone'])){$sNoCheckDrone = $_GET['drone'];} // DroneBL
	if(isset($_GET['tvo'])){$sNoCheckTVO = $_GET['tvo'];} // Tornevall.org
	if(isset($_GET['efn'])){$sNoCheckEFN = $_GET['efn'];} // EFNet
	if(isset($_GET['tor'])){$sNoCheckTor = $_GET['tor'];} // Tor
	if(isset($_GET['dach'])){$sNoCheckDroneACH = $_GET['dach'];} // Drone.abuse.ch
	if(isset($_GET['sach'])){$sNoCheckSpamACH = $_GET['sach'];} // spam.abuse.ch
	if(isset($_GET['hach'])){$sNoCheckHTTPBLACH = $_GET['hach'];} // HTTPBL.abuse.ch
	if(isset($_GET['zach'])){$sNoCheckZeusACH = $_GET['zach'];} // ZeusTracker.abuse.ch
	if(isset($_GET['blde'])){$sNoCheckBLDE = $_GET['blde'];} // Blocklist.de
	...
	if(!$sSFSAPI==""){
		...
		echo "Required StopForumSpam API is missing";
	} // End if(!$sSFSAPI ...
	...
	if ($ip !='' && $sCheckDNSBL =='' && $bFoundMatch==false){
		...
		//
		$sBLDETemp = $rev.'.all.bl.blocklist.de';
		$lookup = gethostbyname($sBLDETemp);
		if ($lookup !=$sBLDETemp)
		{
			switch($sBLDETemp){
				case "127.0.0.2":
			...
			if($sBLDEspambot == true){
				echo "Blocklist.de (" . $sBLDE . ")";
			}
		}

		if($sbldespambot == true || $sTorspambot == true || $sefnetspambot == true || $stvospambot == true || $sphpspambot ==true || $sorbsspambot ==true || $spamhausspambot ==true || $scopspambot || $sdronespambot==true || $httpblachspambot == true || $droneachspambot == true || $spamachspambot == true || $zeusachspambot == true){
			$spambot = true; // Required seperately now that dumping to a text file is optional
		}
	} // End if ($ip !='')
	// *********************************************************************************
	...
	if($spambot ==true){
		if($sbldespambot == true){
			$spambot = true;
			if($bln_SaveToFile == true){$lRet = LogSpammerToFile($savetofolder, 'Blocklist.de',$name, $ip, $mail);}
			if($bln_SaveToDB == true){$lRet = LogSpammerToDB($dbShost, $dbSname, $dbSusername, $dbSpassword, 'Blocklist.de', $name, $ip, $mail);}
...
if(isset($_GET['email']) && $_GET['email'] !='' || isset($_GET['ip']) && $_GET['ip'] !='' || isset($_GET['name']) && $_GET['name'] !=''){
	...
	//}else{
		...
		if(isset($_GET['email'])){$sMail=$_GET['email'];}else{$sMail="";}
		if(isset($_GET['ip'])){$sIP=$_GET['ip'];}else{$sIP="";}
		if(isset($_GET['name'])){$sName=$_GET['name'];}else{$sName="";}
		...
		if(!$sName==""){
			$sName = addslashes(htmlentities($sName)); $sName = urlencode($sName);
		}else{
			// If the email and IP are also missing, we can't do anything
			if($sMail=="" && $sIP==""){
				die('VARS_MISSING TRUE');
			}
		}
