...
//
//		Contains a modification to LogSpammerToDB() with thanks to Jay Riley (jayriley.com)
//
...
// Left and right functions (knew there was a reason I hated PHP - there's no simple ASP equiv's for most of it!)
//
//	Found at: http://www.dreamincode.net/code/snippet2302.htm
//
...
function IsValidEmail($sMailToCheck){
	// If first char is @, ? or * it's invalid
	if(left($sMailToCheck, 1) == "@" || left($sMailToCheck, 1) == "?" || left($sMailToCheck, 1) == "*"){
		return FALSE;
	}else{
...
function GetSpammerCount($m_opt){
	if($m_opt=='F_TEXT'){
		...
		include('config.php');
		$conSBST = @mysqli_connect($dbShost,$dbSusername,$dbSpassword);
		if(!$conSBST){
			return $l_ret;
		}else{
			$m_ret = @mysqli_select_db($conSBST, $dbSname);
			if(!$m_ret=="0"){
				$sSQL = 'SELECT fldspammer_id From tblspammers';
				$sQuery = @mysqli_query($$conSBST, sSQL);
				if($sQuery){
					$l_ret = @mysqli_num_rows($sQuery);
				}
			}
			mysqli_close($conSBST);
		}
	}
	if($l_ret==''){$l_ret="0";}
	return $l_ret;
...
function IsSpammerInDB($host, $dbname, $username, $password, $spammername, $spammerip, $spammeremail){
	...
	$conSBST = mysqli_connect($host, $username, $password);
	if(!$conSBST){
		return false;
	}else{
		// Check our database exists shall we?
		$ret = mysqli_select_db($conSBST, $dbname);
		if($ret == "0"){
		...
		$spammername=mysqli_real_escape_string($conSBST, $spammername);

		$spammerip=str_replace(array("0x", ",", "%", "'","\r\n", "\r", "\n"), "", $spammerip);
		$spammerip=mysqli_real_escape_string($conSBST, $spammerip);

		$spammeremail=str_replace(array("0x", ",", "%", "'","\r\n", "\r", "\n"), "", $spammeremail);
		$spammeremail=mysqli_real_escape_string($conSBST, $spammeremail);
		...
		$sSQL = "SELECT fldspammer_id, fldspammer_name, fldspammer_count, fldspammer_ip, fldspammer_mail FROM tblspammers WHERE fldspammer_name='$spammername' OR fldspammer_ip='$spammerip' OR fldspammer_mail='$spammeremail'";
		$sQuery = mysqli_query($conSBST, $sSQL);
		if(!$sQuery){
			die(mysqli_error($conSBST));
		}else{
...
function LogSpammerToDB($host, $dbname, $username, $password, $spamdb, $spammername, $spammerip, $spammeremail){
	...
	if(!$conSBST){
		...
		// Check to see if our spammer already exists (compares username/IP/E-mail AND the database they are listed in, returns false if only one matches)
		$sSQL = "SELECT fldspammer_id, fldspammer_name, fldspammer_count, fldspammer_ip, fldspammer_mail FROM tblspammers WHERE (";

			if ($spammername != 'NULL'){
				$sSQL .= "fldspammer_name='$spammername'";

				if (($spammeremail != 'NULL') || ($spammerip != 'NULL')){
					$sSQL .=  " OR ";
				}
			}

			if ($spammeremail != 'NULL'){
				$sSQL .= "fldspammer_mail='$spammeremail'";
			...
			if ($spammerip != 'NULL'){
				$sSQL .= "fldspammer_ip='$spammerip'";
			}

			$sSQL .= ") AND flddb_matched='$spamdb'";
...
function CreateDatabase($host, $username, $password, $dbname){
	...
	if(!$conSBST){
		...
		if (!mysqli_query($conSBST, "CREATE DATABASE ".$dbname)){
			return mysqli_error($conSBST);
		}

		// Create table if it doesn't exist
		mysqli_select_db($conSBST, $dbname);
