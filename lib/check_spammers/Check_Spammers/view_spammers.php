...
<?php
	...
	if($bln_SaveToDB==True && $_SESSION['type']!="TEXT"){
		$conView = mysqli_connect($dbShost, $dbSusername, $dbSpassword);
		if(!$conView){
			echo '<tr><td class="results_white" align="center" colspan="6">'.mysqli_error().'</td></tr>';
		}else{
			$rView = mysqli_select_db($conView, $dbSname);
			// Try first to ensure it exists, if not, try and create it
			if($rView =="0"){
				mysqli_close($conView);
				$rRet = CreateDatabase($dbShost, $dbSusername, $dbSpassword, $dbSname);
				// Did it fail?
				$conView = mysqli_connect($dbShost, $dbSusername, $dbSpassword);
				$rRet = mysqli_select_db($conView, $dbSname);
				if($rRet=="0"){
					die('<tr><td class="results_white" align="center" colspan="6">Error: '.mysqli_error().'</td></tr>');
				}
			}
			// Last try, if it still doesn't exist, notify the user of the problem
			$rView = mysqli_select_db($conView, $dbSname);
			if($rView == "0"){
				echo '<tr><td class="results_white" align="center" colspan="6">Error: '.mysqli_error().'</td></tr>';
			}else{
				...
				$sQuery = mysqli_query($conView, $sSQL);
				if(!$sQuery){
					echo '<tr><td class="results_white" align="center" colspan="6">Error: '.mysqli_error($conView).'</td></tr>';
				}else{

					$num = mysqli_num_rows($sQuery); $lTotalNum = $num;
